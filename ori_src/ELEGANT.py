...
class ELEGANT(object):
    ...
    def restore_from_file(self):
        ...
        if (
        ...
        ):  # Check if a checkpoint ID is provided for restoration
            # Construct paths to encoder and decoder checkpoint files
            ckpt_file_enc = os.path.join(
                self.config.model_dir, "Enc_iter_{:06d}.pth".format(self.restore)
            )
            assert os.path.exists(
                ckpt_file_enc
            )  # Ensure the encoder checkpoint file exists
            ckpt_file_dec = os.path.join(
                self.config.model_dir, "Dec_iter_{:06d}.pth".format(self.restore)
            )
            assert os.path.exists(
                ckpt_file_dec
            )  # Ensure the decoder checkpoint file exists
            ...
            if self.gpu:
                ...
                self.Enc.load_state_dict(
                    torch.load(ckpt_file_enc, map_location="cpu"), strict=False
                )
                self.Dec.load_state_dict(
                    torch.load(ckpt_file_dec, map_location="cpu"), strict=False
                )

            # If in training mode, load discriminator parameters
            if self.mode == "train":
                # Construct paths to discriminator checkpoint files
                ckpt_file_d1 = os.path.join(
                    self.config.model_dir, "D1_iter_{:06d}.pth".format(self.restore)
                )
                assert os.path.exists(
                    ckpt_file_d1
                )  # Ensure the D1 discriminator checkpoint file exists
                ckpt_file_d2 = os.path.join(
                    self.config.model_dir, "D2_iter_{:06d}.pth".format(self.restore)
                )
                assert os.path.exists(
                    ckpt_file_d2
                )  # Ensure the D2 discriminator checkpoint file exists
                ...
                if self.gpu:
                    ...
                    self.D1.load_state_dict(
                        torch.load(ckpt_file_d1, map_location="cpu"), strict=False
                    )
                    self.D2.load_state_dict(
                        torch.load(ckpt_file_d2, map_location="cpu"), strict=False
                    )
    ...
    def set_mode_and_gpu(self):
        ...
        if self.mode == "train":
            ...
            self.optimizer_D = torch.optim.Adam(
                chain(self.D1.parameters(), self.D2.parameters()),
                lr=self.config.D_lr,
                betas=(0.5, 0.999),
                weight_decay=self.config.weight_decay,
            )
            ...
            if len(self.gpu) > 1:
                self.Enc = torch.nn.DataParallel(
                    self.Enc, device_ids=list(range(len(self.gpu)))
                )
                self.Dec = torch.nn.DataParallel(
                    self.Dec, device_ids=list(range(len(self.gpu)))
                )
                self.D1 = torch.nn.DataParallel(
                    self.D1, device_ids=list(range(len(self.gpu)))
                )
                self.D2 = torch.nn.DataParallel(
                    self.D2, device_ids=list(range(len(self.gpu)))
                )
    ...
    def forward_G(self):
        ...
        self.z_C = torch.cat(
            [
                self.get_attr_chs(self.z_A, i)
                if i != self.attribute_id
                else self.get_attr_chs(self.z_B, i)
        ...
        self.z_D = torch.cat(
            [
                self.get_attr_chs(self.z_B, i)
                if i != self.attribute_id
                else self.get_attr_chs(self.z_A, i)
                for i in range(self.n_attributes)
            ],
            1,
        )
    ...
    def compute_loss_G(self):
        ...
        self.loss_G = (
            5 * self.G_loss["reconstruction"]
            + self.G_loss["adv1"]
            + 0.5 * self.G_loss["adv2"]
        )
        ...
        for tag, images in image_info.items():
            for idx, image in enumerate(images):
                self.writer.add_image(
                    tag + "/{}_{:02d}".format(self.attribute_id, idx), image, self.step
                )
    ...
    def save_sample_images(self, save_num=5):
        ...
        img_array = np.transpose(
            self.img_denorm(canvas.data.cpu().numpy()), (0, 2, 3, 1)
        ).astype(np.uint8)
        for i in range(save_num):
            Image.fromarray(img_array[i]).save(
                os.path.join(
                    self.config.img_dir,
                    "step_{:06d}_attr_{}_{:02d}.jpg".format(
                        self.step, self.attribute_id, i
                    ),
                )
            )
    ...
    def save_model(self):
        ...
        torch.save(
            {reduced(key): val.cpu() for key, val in self.Enc.state_dict().items()},
            os.path.join(
                self.config.model_dir, "Enc_iter_{:06d}.pth".format(self.step)
            ),
        )
        torch.save(
            {reduced(key): val.cpu() for key, val in self.Dec.state_dict().items()},
            os.path.join(
                self.config.model_dir, "Dec_iter_{:06d}.pth".format(self.step)
            ),
        )
        torch.save(
            {reduced(key): val.cpu() for key, val in self.D1.state_dict().items()},
            os.path.join(self.config.model_dir, "D1_iter_{:06d}.pth".format(self.step)),
        )
        torch.save(
            {reduced(key): val.cpu() for key, val in self.D2.state_dict().items()},
    ...
    def swap(self):
        ...
        images = self.transform(
            Image.open(self.args.input), Image.open(self.args.target[0])
        )
        ...
        img = np.transpose(
            self.img_denorm(img.data.cpu().numpy()), (0, 2, 3, 1)
        ).astype(np.uint8)[0]
    ...
    def linear(self):
        ...
        # Denormalize the image tensor and convert it to a numpy array
        img = np.transpose(
            self.img_denorm(out.data.cpu().numpy()), (0, 2, 3, 1)
        ).astype(np.uint8)[0]
    ...
    def matrix1(self):
        ...
        self.B = self.tensor2var(
            self.transform(Image.open(self.args.input)), volatile=True
        )

        # Load and transform the target images (As)
        self.As = [
            self.tensor2var(
                self.transform(Image.open(self.args.target[i])), volatile=True
            )
            for i in range(3)
        ]
        ...
        self.z_Ds = [
            torch.cat(
                [
                    self.get_attr_chs(self.z_B, i)
                    if i != self.attribute_id
                    else self.get_attr_chs(self.z_As[j], i)
                    for i in range(self.n_attributes)
                ],
                1,
            )
        ...
        # Denormalize the image tensor and convert it to a numpy array
        img = np.transpose(self.img_denorm(canvas.numpy()), (0, 2, 3, 1)).astype(
            np.uint8
        )[0]
    ...
    def matrix2(self):
        ...
        self.B, self.A1, self.A2 = self.tensor2var(
            self.transform(
                Image.open(self.args.input),
                Image.open(self.args.target[0]),
                Image.open(self.args.target[1]),
            ),
            volatile=True,
        )
        ...
        self.z_B, self.B_skip = self.Enc(self.B, return_skip=True)
        self.z_A1, self.z_A2 = (
            self.Enc(self.A1, return_skip=False),
            self.Enc(self.A2, return_skip=False),
        )
        ...
        self.z_D1 = torch.cat(
            [
                self.get_attr_chs(self.z_B, i)
                if i != self.attribute_ids[0]
                else self.get_attr_chs(self.z_A1, i)
                for i in range(self.n_attributes)
            ],
            1,
        )
        ...
        self.z_D2 = torch.cat(
            [
                self.get_attr_chs(self.z_B, i)
                if i != self.attribute_ids[1]
                else self.get_attr_chs(self.z_A2, i)
                for i in range(self.n_attributes)
            ],
            1,
        )
        ...
        # Create a canvas for visualization
        first_col = torch.cat(
            (self.B.data.cpu(), torch.ones(1, 3, (m - 2) * h, w), self.A1.data.cpu()),
            -2,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help='Specify attribute names. Use "all" for all attributes.',
    )
    parser.add_argument(
        "-g", "--gpu", default=[], nargs="+", type=str, help="Specify GPU ids."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--matrix",
        action="store_true",
        help="Matrix interpolation with respect to one attribute.",
    )
    parser.add_argument(
        "--swap_list",
        default=[],
        nargs="+",
        type=int,
        help="Specify the attributes ids for swapping.",
    )
    parser.add_argument("-i", "--input", type=str, help="Specify the input image.")
    parser.add_argument(
        "-t", "--target", nargs="+", type=str, help="Specify target images."
    )
    parser.add_argument(
        "-s", "--size", nargs="+", type=int, help="Specify the interpolation size."
    )

    args = parser.parse_args()

    # Handle "all" for attributes
    if args.attributes and args.attributes[0].lower() == "all":
        from dataset import all_attrs  # Import the all_attrs variable

        args.attributes = all_attrs
